...
Watch the *count* per namespace. smithay's `LayerMap::arrange` reserves the exclusive zone of every layer in the map whether or not it has a buffer, so each extra surface shrinks the app area — four live wvkbd surfaces once took the usable height down to 117px. `DEAD` means the objects are gone but the map still holds the surface (never yet observed); several *alive* surfaces in one namespace means the client leaked them, and `id=` ties each row back to the `layer surface created/destroyed` log lines. A create logged with `already_in_namespace` above 0 is the leak happening.

wvkbd 0.20 leaks this way: `show()` guards on its `layer_surface` global but assigns it only *after* `refresh_available_dimension()` has run two `wl_display_roundtrip()`s. Anything dispatched from those round-trips that re-enters `show()` — an `im_activate`, or `layer_surface_configure`'s `hide(); show()` restart when the configure size differs from what was asked for — orphans the surface the outer call is halfway through building: mapped, drawn, holding its exclusive zone, unreachable by `hide()`.

It runs away because each orphan's zone shrinks the usable area, which changes the next configure (so another restart) and eventually makes `redimension_keyboard()` read `available_width > available_height` and switch to the 120px *landscape* layout — the giveaway in a dump is wvkbd rows at mixed 250/120 heights. Seen going 2525 → 1825 → 453 → 117px in one emoji-picker press. Patch in `nix/wvkbd-show-reentrancy.patch`.
