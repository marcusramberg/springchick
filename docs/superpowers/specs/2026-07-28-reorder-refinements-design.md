...
- **Dock → grid drop:** honors the hovered grid slot (the answer to the drop-location question). Just `move_to(app, hovered_page, hovered_index)` — the reworked `move_to` strips the app from `dock` (and `hidden`) directly before inserting, so no separate `unpin` is needed. The live grid gap opens under the finger during the drag exactly as for a grid-sourced reorder.
