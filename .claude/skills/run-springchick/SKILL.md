...
Count the surfaces per namespace. `arrange` reserves each one's `excl=` zone
even with `buf=none`, so extras shrink the usable area (seen on device at 117px
with four `wvkbd` surfaces). A pile of *alive* ones means the client leaked
them — match `id=` against the `layer surface created/destroyed` lines in the
compositor log, where a create with `already_in_namespace>0` is the leak itself.
`DEAD` would mean the objects died with the surface still mapped; not yet seen.
